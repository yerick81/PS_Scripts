...
# Check if Microsoft Monitoring Agent file exists, if not, download it
 ...
 else
 {
     ...
     $URLs = "http://download.microsoft.com/download/1/5/E/15E274B9-F9E2-42AE-86EC-AC988F7631A0/MMASetup-AMD64.exe"

     (New-Object Net.WebClient).DownloadFile($URLs, $MMAFile)
...
# Check if Service Map Agent exists, if not, download it
 ...
 else
 {
     ...
     $URLs = "https://aka.ms/dependencyagentwindows"

     (New-Object Net.WebClient).DownloadFile($URLs, $SMFile)
